...
def _scrape_google_news_rss(timeout=8):
    ...
    Returns list of dicts: [{"text": str, "source": "Google News RSS"}, ...]
    """
    headlines = []
    seen = set()
            ...
            for item in items:
                ...
                if title:
                    ...
                    if text and len(text) > 10 and text not in seen:
                        seen.add(text)
                        headlines.append({"text": text, "source": "Google News RSS"})
...
def scrape_psx_headlines(timeout=8):
    ...
    Returns list of dicts: [{"text": str, "source": str}, ...]
    """
    headlines = []
    for src in _SOURCES:
        try:
            resp = requests.get(
                src["url"], headers=_HEADERS, timeout=timeout
            )
            if resp.status_code != 200:
                logger.warning(f"{src['name']}: HTTP {resp.status_code}")
                continue
            soup = BeautifulSoup(resp.content, "html.parser")
            tags = soup.select(src["selector"])
            for tag in tags:
                text = tag.get_text(strip=True)
                if text and len(text) > 10:  # skip tiny fragments
                    headlines.append({"text": text, "source": src["name"]})
            logger.info(f"{src['name']}: scraped {len(tags)} headlines")
        except Exception as e:
            logger.warning(f"{src['name']} scrape failed: {e}")
...
# Sentiment Analysis
# ---------------------------------------------------------------------------
def analyze_sentiment(headline_dicts):
    """Run VADER on each headline dict, return (mean_score, scored_list).
    headline_dicts: [{"text": str, "source": str}, ...]
    Returns:
        mean_score (float): -1.0 to +1.0
        scored (list[dict]): [{"text", "source", "score"}, ...] sorted by |score| desc
    """
    ...
    for h in headline_dicts:
        compound = vader.polarity_scores(h["text"])["compound"]
        scored.append({
            "text": h["text"],
            "source": h["source"],
            "score": round(compound, 4),
        })
    # Sort by absolute score descending so most impactful headlines appear first
    scored.sort(key=lambda x: abs(x["score"]), reverse=True)
    mean_score = sum(s["score"] for s in scored) / len(scored)
    return round(mean_score, 4), scored


# ---------------------------------------------------------------------------
# Cached Orchestrator
# ---------------------------------------------------------------------------
_cache = {"score": 0.0, "label": "Neutral", "count": 0,
          "headlines": [], "ts": 0}
...
def _build_response():
    """Build the public response dict from the current cache."""
    return {
        "score": _cache["score"],
        "label": _cache["label"],
...
def get_live_sentiment():
    ...
    with _cache_lock:
        ...
        try:
            headline_dicts = scrape_psx_headlines()
            score, scored_headlines = analyze_sentiment(headline_dicts)
